...
class AutoTrader:
    ...
    async def execute_trade(self, symbol, strategy, amount=None, price=None, use_ai_price=False):
        ...
        try:
            ...
            if available_durations and duration not in available_durations:
                ...
                if available_durations:
                    ...
                    closest_duration = min(available_durations, key=lambda x: (abs(x - duration), -x))
                    duration = closest_duration

                    logger.info(f"Adjusted duration from {original_duration}s to {duration}s for {symbol} (supported durations: {available_durations})")
        ...
        if use_ai_price and not price:
            ...
            try:
                recent_candles = await self.client.get_candles(symbol, 60, count=10)  # Get recent 1-minute candles
                if recent_candles and len(recent_candles) > 0:
                    ...
                    if contract_type == 'CALL':
                        ...
                        logger.info(f"AI price management: Setting CALL price to {trade_price} (market: {latest_price})")
                    else:  # PUT
                        # For selling, try to get a better exit price (0.1% above market)
                        trade_price = round(latest_price * 1.001, 4)
                        logger.info(f"AI price management: Setting PUT price to {trade_price} (market: {latest_price})")
            except Exception as e:
        ...
        while True:
            try:
                ...
                if entry_price is None:
                    ...
                    duration = contract_info.get("date_expiry", 0) - contract_info.get("date_start", 0)
                    symbol = contract_info.get("underlying", "")

                # Get current price and profit/loss
                current_spot = contract_info.get("current_spot", 0)
                current_spot_time = contract_info.get("current_spot_time", 0)
                ...
                self._notify_status({
                    "status": "error", 
                    "message": f"Error checking trade status: {str(e)}"
                })
                ...
                if retry_count >= max_retries:
                    logger.warning(f"Reached maximum retries ({max_retries}) for checking trade status. Continuing monitoring.")
                    retry_count = 0  # Reset counter to avoid continuous warnings

                # Wait before retrying
                await asyncio.sleep(5)

    async def trading_cycle(self, symbol, granularity, manual_price=None, use_ai_price=False, ai_managed_trading=False):
        """
        ...
        if not self.check_trading_conditions():
            ...
            if self.last_model_training:
                hours_since_training = (datetime.datetime.now() - self.last_model_training).total_seconds() / 3600
        ...
        threshold_to_use = self.custom_confidence_threshold if self.custom_confidence_threshold is not None else CONFIDENCE_THRESHOLD
        if strategy.get('confidence', 0) < threshold_to_use:
            logger.info(f"Strategy confidence too low: {strategy.get('confidence', 0):.2f} < {threshold_to_use}")
            self._notify_status({
                "status": "skipped", 
        ...
        if ai_managed_trading:
            ...
            try:
                ...
                if recent_candles and len(recent_candles) > 10:
                    ...
                    price_ranges = [(h - l) / avg_price for h, l in zip(highs, lows)]
                    ...
                    trade_amount = round(max(5.0, min(optimal_amount, self.base_trade_amount * 2)), 2)

                    logger.info(f"AI-managed trading: Adjusted trade amount to ${trade_amount} " +
                    ...
                    if strategy:
                        ...
                        if volatility > 0.02:  # Very high volatility
                            strategy['duration'] = max(30, int(strategy['duration'] * 0.7))  # Shorter duration
                        elif volatility < 0.005:  # Low volatility
                            strategy['duration'] = min(300, int(strategy['duration'] * 1.3))  # Longer duration

                        logger.info(f"AI-managed trading: Adjusted trade duration to {strategy['duration']}s")
            except Exception as e:
        ...
        contract_id = await self.execute_trade(
            ...
            amount=trade_amount,
            price=manual_price,
            use_ai_price=use_ai_price or ai_managed_trading  # Enable AI price if either option is on
        )
        ...
        if self.trade_count % 10 == 0:
            ...
            logger.info(f"Performance report after {self.trade_count} trades: "
    ...
    async def run_trading_loop(self, symbol, granularity, manual_price=None, use_ai_price=False, ai_managed_trading=False, confidence_threshold=None):
        """
        ...
        self._notify_status({
            ...
            "settings": {
                ...
                "confidence_threshold": self.custom_confidence_threshold if self.custom_confidence_threshold is not None else CONFIDENCE_THRESHOLD,
        ...
        logger.info(f"Trading session ended. Final report: Win rate: {final_report['summary']['win_rate']:.2f}, "
                   f"Net profit: ${final_report['summary']['net_profit']:.2f}")

    def start_trading(self, symbol, granularity, manual_price=None, use_ai_price=False, ai_managed_trading=False, confidence_threshold=None):
        """
        ...
        self.trading_thread = threading.Thread(
            ...
            args=(self.loop, symbol, granularity, manual_price, use_ai_price, ai_managed_trading, confidence_threshold),
            daemon=True
        )
    ...
    def _run_trading_thread(self, loop, symbol, granularity, manual_price=None, use_ai_price=False, ai_managed_trading=False, confidence_threshold=None):
        """Run the trading loop in a separate thread."""
        asyncio.set_event_loop(loop)
        loop.run_until_complete(self.run_trading_loop(symbol, granularity, manual_price, use_ai_price, ai_managed_trading, confidence_threshold))
    ...
    def get_status(self):
        ...
        return {
            ...
            "session_duration": str(datetime.datetime.now() - self.session_start_time).split('.')[0]
        }
    ...
    def log_trade(self, trade_record):
        ...
        log_file = os.path.join(self.logs_dir, f"trades_{datetime.datetime.now().strftime('%Y%m%d')}.json")
    ...
    def adjust_trade_size(self):
        ...
        logger.info(f"Trade size adjusted: ${self.current_trade_amount} (multiplier: {self.trade_size_multiplier:.2f})")
    ...
    def check_trading_conditions(self):
        ...
        if self.last_model_training:
            hours_since_training = (datetime.datetime.now() - self.last_model_training).total_seconds() / 3600
            if hours_since_training > MODEL_UPDATE_FREQUENCY:
                logger.info(f"Model update needed: {hours_since_training:.1f} hours since last training")
                return False
    ...
    def get_performance_report(self):
        ...
        best_hour = max(self.performance_by_hour.items(), 
                        key=lambda x: x[1]['wins']/x[1]['trades'] if x[1]['trades'] > 0 else 0)
        worst_hour = min(self.performance_by_hour.items(), 
        ...
        if self.performance_by_market:
            best_market = max(self.performance_by_market.items(), 
                             key=lambda x: x[1]['wins']/x[1]['trades'] if x[1]['trades'] > 0 else 0)
            worst_market = min(self.performance_by_market.items(), 
                              key=lambda x: x[1]['wins']/x[1]['trades'] if x[1]['trades'] > 0 else 1)
        ...
        report = {
            ...
            "hourly_performance": {
                "best_hour": {
                    "hour": best_hour[0],
                    "win_rate": best_hour[1]['wins']/best_hour[1]['trades'] if best_hour[1]['trades'] > 0 else 0,
                    "trades": best_hour[1]['trades']
                },
                "worst_hour": {
                    "hour": worst_hour[0],
                    "win_rate": worst_hour[1]['wins']/worst_hour[1]['trades'] if worst_hour[1]['trades'] > 0 else 0,
        ...
        if best_market:
            report["market_performance"]["best_market"] = {
                ...
                "win_rate": best_market[1]['wins']/best_market[1]['trades'] if best_market[1]['trades'] > 0 else 0,
        ...
        if worst_market:
            report["market_performance"]["worst_market"] = {
                ...
                "win_rate": worst_market[1]['wins']/worst_market[1]['trades'] if worst_market[1]['trades'] > 0 else 0,
