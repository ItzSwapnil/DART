"""
DART v2.0 - Modern Web Dashboard
A stunning, professional web-based interface featuring glassmorphism design,
real-time AI insights, and advanced trading analytics.
"""
...
class ModernDashboard:
    ...
    def render_header(self):
        ...
        st.markdown(
            '<p style="text-align: center; color: #94a3b8; font-size: 1.2rem; margin-bottom: 2rem;">Deep Adaptive Reinforcement Trader - Professional Trading Platform</p>',
            unsafe_allow_html=True,
        )
    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            ...
            if (
                "selected_market" not in st.session_state
                or st.session_state.selected_market != selected_market
            ):
            ...
            use_deep_rl = st.checkbox(
                "Enable Deep RL", value=USE_DEEP_RL, help="Use Deep Reinforcement Learning",
            )
            use_enhanced_features = st.checkbox(
                "Enhanced Features",
                value=USE_ENHANCED_FEATURES,
                help="Use multi-modal feature extraction",
            )
            ...
            max_daily_loss = st.number_input(
                "Max Daily Loss ($)", value=MAX_DAILY_LOSS, min_value=10.0, max_value=1000.0,
            )
            trade_amount = st.number_input(
                "Trade Amount ($)", value=TRADE_AMOUNT, min_value=1.0, max_value=100.0,
            )
    ...
    def render_ai_insights(self):
        ...
        if self.trading_ai and hasattr(self.trading_ai, "get_market_analysis"):
            try:
                # Get current market from session state
                market = st.session_state.get("selected_market", "EUR/USD")
                market_data = self.get_market_data(market, "1m", 100)
                if not market_data.empty:
                    analysis = self.trading_ai.get_market_analysis(market_data.to_dict("records"))
                    if analysis:
                        # Extract sentiment from RSI/momentum
                        rsi = analysis.get("momentum", {}).get("rsi", 50)
                        sentiment_score = rsi / 100

                        # Get regime if available
                        regime_info = analysis.get("market_regime", {})
                        regime_name = regime_info.get("regime_name", "Normal")
            except Exception as e:
        ...
        with col1:
            ...
            <div class="trading-card">
                ...
                <span style="font-size: 1.5rem; color: #10b981;">{(1 - uncertainty) * 100:.1f}%</span>
            </div>
            """,
                unsafe_allow_html=True,
            )
        with col2:
            ...
            <div class="trading-card">
                ...
                <span style="font-size: 1.2rem; color: #8b5cf6;">{regime_name}</span>
            </div>
            """,
                unsafe_allow_html=True,
            )
        ...
        # Get real signals if strategy exists
        if self.trading_ai and self.trading_ai.strategy:
            strategy = self.trading_ai.strategy
            signal = strategy.get("direction", "HOLD")
            confidence = strategy.get("confidence", 0.5) * 100
            unc = strategy.get("uncertainty", {}).get("total", 0.2) * 100

            color = "#10b981" if signal == "CALL" else "#ef4444" if signal == "PUT" else "#f59e0b"
            st.markdown(
                f"""
            <div class="trading-card" style="border-left: 4px solid {color};">
                <span style="color: {color}; font-weight: bold; font-size: 1.3rem;">{signal}</span>
                <span style="float: right; color: #94a3b8;">Just now</span><br>
                Confidence: <strong>{confidence:.1f}%</strong> | Uncertainty: {unc:.1f}%
            </div>
            """,
                unsafe_allow_html=True,
            )
    ...
    def render_performance_metrics(self):
        ...
        df_perf = pd.DataFrame({"date": dates, "portfolio_value": portfolio_values})
